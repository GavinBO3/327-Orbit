...
void setupScene1() {
  ...
  earth = new Orbiter(sun, 50, 0, 1, Orbiter.Type.CIRCLE, Color.blue);
...
void updateOrbiters(double timeDelta) {
  ...
  LinkedList<Orbiter> q = new LinkedList<Orbiter>();
  q.add(sun);
  
  for (int i = 0; i < q.size(); i++) {
    q.get(i).updateRotation(timeDelta);
    Iterator<Orbiter> currList = q.get(i).getChildren().iterator(); 
...
void drawOrbiter(Orbiter orbiter) {
  try {
    ...
    System.out.println("New position matrix: \n" + position + "\n\n\n\n");
