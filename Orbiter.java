...
public class Orbiter {
  ...
  public Matrix getMatrix() throws UndefinedMatrixOpException {
    ...
    Matrix angle = Matrix.rotationH2D(getOrbitAngle());
    Matrix trans = Matrix.translationH2D(getOrbitRadius(), 0);
    
    System.out.println("Angle matrix: \n" + angle + "\nTrans Matrix: \n" + trans);
    

    
    Matrix Parent = getParent().getMatrix();
    System.out.println("Parent matrix: \n" + Parent);
    

        Matrix R = Parent.dot(angle);
